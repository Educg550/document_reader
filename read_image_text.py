...
def get_llm_response(raw_text, conversation):
  response = conversation.generate_content(
      "Based on the following text excerpt from an electricity bill, answer whether the energy company used is ENEL or CEMIG, answer JUST 'ENEL' OR 'CEMIG'. The text is in Portuguese:\n\n" + raw_text)
...
def get_text_from_pdf(file_path):
  ...
  for page in pages:
    ...
    document = get_text_from_image(file_name)
    for tup in document:
      result += tup[1] + " "
    result += "\n"
  return result
...
def main():
  ...
  with st.sidebar:
    ...
    uploaded_file = st.file_uploader("Faça o upload de uma imagem, PDF ou texto puro e clique em Detectar", type=[
        "txt", "pdf", "jpg", "jpeg", "png"], accept_multiple_files=False)
    if st.button("Detectar companhia de energia"):
      with st.spinner("Detectando companhia de energia..."):
        if uploaded_file is not None:
          ...
          if show_vision:
            st.write(
                "Predição da companhia de energia por visão computacional: ", vision_predict)
          if show_llm:
            st.write(
                "Predição da companhia de energia pela LLM (Gemini): ", llm_predict)
          st.session_state.finished_processing = True
        else:
          st.error("Nenhum arquivo foi enviado")

  if show_raw_text and st.session_state.finished_processing:
    st.session_state.finished_processing = False
